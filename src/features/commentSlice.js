 ...
 * 댓글 좋아요, id로 특정 댓글 구분,
 */
export const likeCommentThunk = createAsyncThunk('comment/likeComment', async ({ commentId, userId }, { rejectWithValue }) => {
   try {
   ...
   extraReducers: (builder) => {
      builder
         ...
         .addCase(likeCommentThunk.rejected, (state, action) => {
            state.loading = false
            state.error = action.payload?.message || '서버 문제로 댓글을 수정하지 못했습니다.'
         })
   },
})
