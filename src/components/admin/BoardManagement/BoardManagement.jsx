...
const BoardManagement = () => {
  ...
  const handlePostAction = (post, action) => {
    ...
    if (action === 'view') {
      ...
      setPosts(posts.map(p => 
        p.id === post.id ? { ...p, isNotice: !p.isNotice } : p
      ));
    }
  };
        ...
        {selectedPost && (
          <div>
              ...
              <Col>
                ...
                <div className="mb-2">
                  ...
                  {selectedPost.isNotice && (
                    <Badge bg="primary" className="ms-2">공지</Badge>
                  )}
                  {selectedPost.isPinned && (
                    <Badge bg="info" className="ms-2">고정</Badge>
                  )}
                </div>
            ...
            <Row className="mb-3">
              ...
              <Col md={3}>
                <div className="text-center p-3 bg-light rounded">
                  <div className={`h4 mb-1 ${selectedPost.reports > 0 ? 'text-danger' : ''}`}>
              ...
              <tbody>
                {sortedPosts.map(post => (
                  <tr key={post.id}>
                    ...
                    <td>{post.comments}</td>
                    <td>
                      <span className={post.reports > 0 ? 'text-danger fw-bold' : ''}>
                        {post.reports}
                      </span>
                    </td>
                    ...
                    <td>
                      ...
                      {post.status === 'published' ? (
                        ...
                        <Button
                          ...
                          size="sm"
                          className={`${styles.actionButton} me-1`}
                          onClick={() => handlePostAction(post, 'show')}
                        >
                          <i className="fas fa-eye"></i>
                        </Button>
                      ) : null}
